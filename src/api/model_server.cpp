...
//        tvaImg.pData =
        ...
        Model::InferenceParam inferParam;
        Model::InferenceResult inferRes;
        inferParam.nmsThreshold = _param.nms_threshold();
        inferParam.threshold = _param.threshold();

        inferParam.pData = _data.pic_data();
        inferParam.tvimage = request->data().pic_data();
        Model::STATUS status = manager.Inference(request->model_enum(),&inferParam,&inferRes);
        if (status!=Model::SUCCESS){
            reply->set_ret_code(Model::ERROR);
        }
